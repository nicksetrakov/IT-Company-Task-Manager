...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for position_name in positions:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f'Successfully created position: {position.name}'))
            else:
                self.stdout.write(self.style.WARNING(f'Position already exists: {position.name}'))
        ...
        for task_type_name in task_types:
            task_type, created = TaskType.objects.get_or_create(name=task_type_name)
            if created:
                self.stdout.write(self.style.SUCCESS(f'Successfully created task type: {task_type.name}'))
            else:
                self.stdout.write(self.style.WARNING(f'Task type already exists: {task_type.name}'))
        ...
        for _ in range(10):
            ...
            for _ in range(random.randint(1, 5)):
                ...
                deadline = timezone.now() + timezone.timedelta(days=random_days)
                is_completed = fake.boolean(chance_of_getting_true=50)
                priority = random.choice([choice[0] for choice in Priority.choices])
                task_type = TaskType.objects.get(name=random.choice(task_types))
                tags = Tag.objects.order_by('?')[:random.randint(1, 3)]

                task = Task.objects.create(name=name, description=description, deadline=deadline, is_completed=is_completed, priority=priority, task_type=task_type)
